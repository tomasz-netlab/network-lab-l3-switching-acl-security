...
| F-18 | HR / FINANCE | own SVI | ICMP | echo | PERMIT | Users may ping their own default gateway (basic connectivity check) |
...
| ACL_HR_IN | SW-DISTRO | SVI VLAN 10 | Inbound | F-09, F-10, F-11, F-18 |
| ACL_FINANCE_IN | SW-DISTRO | SVI VLAN 20 | Inbound | F-07, F-08, F-18 |
...
| Seq | Action | Protocol | Source | Destination | Flags | Flow | Comment |
| --- | ------ | -------- | ---------------- | ----------------- | ----- | ---- | ------- |
| 5 | permit | icmp | 192.168.10.0/27 | host 192.168.10.1 echo | | F-18 | Ping own gateway (diagnostics) |
...
| 45 | permit | udp | host 0.0.0.0 | host 255.255.255.255 eq 67 | | F-11 | DHCP Discover (client has no address yet) |
| 50 | permit | udp | 192.168.10.0/27 eq 68 | 10.0.120.0/28 eq 67 | | F-11 | DHCP renewal (client already addressed) |
...
| Seq | Action | Protocol | Source | Destination | Flags | Flow | Comment |
| --- | ------ | -------- | ---------------- | ----------------- | ----- | ---- | ------- |
| 5 | permit | icmp | 192.168.20.0/27 | host 192.168.20.1 echo | | F-18 | Ping own gateway (diagnostics) |
...
| 35 | permit | udp | host 0.0.0.0 | host 255.255.255.255 eq 67 | | F-08 | DHCP Discover (client has no address yet) |
...
| 60 | permit | udp | 192.168.20.0/27 eq 68 | 10.0.120.0/28 eq 67 | | F-08 | DHCP renewal (client already addressed) |
...
| 155 | permit | udp | 10.0.120.0/28 eq 67 | 192.168.10.0/27 eq 67 | | F-14 |
| 156 | permit | udp | 10.0.120.0/28 eq 67 | 192.168.20.0/27 eq 67 | | F-13 |
...
### DHCP through a relay needs three separate rules, not one

DHCP looks like a single client-to-server flow, but with a relay agent in the path it is actually three distinct patterns, each needing its own ACE. This only became visible once the ACLs were genuinely enforcing — before that, DHCP worked simply because nothing was filtering it.

**1. Initial Discover — source `0.0.0.0`.** A client that has no address yet sends its Discover from `0.0.0.0` to the broadcast address. It does *not* match a rule scoped to the zone subnet, because the client is not yet in that subnet from the ACL's point of view. Hence the explicit `permit udp host 0.0.0.0 host 255.255.255.255 eq 67`.

**2. Renewal — source is the client's own address.** Once addressed, the client renews by unicast from its real IP, source port 68 to destination port 67. This is the "textbook" rule and the one most examples show.

**3. Server reply to the relay — port 67 to port 67.** This is the one that is easy to miss. The server does not answer the client directly; it answers the *relay agent* (the SVI), and that leg of the conversation runs **67 → 67**, not 67 → 68. Port 68 only appears on the final hop from relay to client, inside the VLAN. A rule written as `eq 67 → eq 68` therefore never matches the server's reply, and the Offer is silently dropped by the RFC 1918 deny block further down the list.

The lesson generalises: when a protocol passes through a relay or proxy, the port pairs on each leg are not the same, and an ACL written from the end-to-end mental model will block the middle leg.

### Users may ping their own gateway

`ACL_HR_IN` and `ACL_FINANCE_IN` each begin with a rule permitting ICMP echo from the zone to its own SVI address. Without it, a user cannot even ping their default gateway — the packet enters the SVI, finds no matching permit, and falls through to the closing deny.

This is a deliberate relaxation. Being able to ping the gateway is the most basic connectivity check available to a user or a helpdesk, and the traffic never leaves the local subnet boundary, so the security cost is negligible. It is scoped tightly: `host 192.168.10.1` and `host 192.168.20.1` respectively, not the whole management or server range.
...
- **ACL application on SVIs** — `ip access-group` applied to an SVI on a multilayer switch does not persist in `show running-config` and stops filtering after a reload. Re-applying the command restores enforcement (confirmed by rule hit counters) even though the interface still reports "Inbound access list is not set". This is a known Packet Tracer defect on the 3560/3650 platforms, reported on the Cisco Community forum; the same configuration behaves normally on real hardware.

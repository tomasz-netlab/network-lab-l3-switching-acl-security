...
SW-DISTRO is configured as the **root bridge for the production VLANs** (10, 20, 101, 123) using the `spanning-tree vlan 10,20,101,123 root primary` macro, which sets the bridge priority to 24576. The priority moves in steps of 4096 because the Extended System ID reserves the lower 12 bits of the Bridge ID for the VLAN number — 24576 is low enough to win the root election against the switches left at the 32768 default. With the root at the distribution layer, both of its downlinks stay forwarding, and STP blocks one end of the SW1↔SW2 base to break the loop. In this build, SW1's Gi0/2 end settles into blocking for the production VLANs while SW2's end stays designated — verified with `show spanning-tree`.

...
ip access-list extended ACL_HR_IN
 5 permit icmp 192.168.10.0 0.0.0.31 host 192.168.10.1 echo
 10 permit icmp 192.168.10.0 0.0.0.31 10.0.100.0 0.0.0.31 echo-reply
 ...
 40 permit udp 192.168.10.0 0.0.0.31 10.0.120.0 0.0.0.15 eq 53
 45 permit udp host 0.0.0.0 host 255.255.255.255 eq 67
 50 permit udp 192.168.10.0 0.0.0.31 eq 68 10.0.120.0 0.0.0.15 eq 67
 60 permit tcp 192.168.10.0 0.0.0.31 10.0.120.0 0.0.0.15 eq 25
 70 permit tcp 192.168.10.0 0.0.0.31 10.0.120.0 0.0.0.15 eq 110
 80 permit tcp 192.168.10.0 0.0.0.31 10.0.120.0 0.0.0.15 eq 143
 191 deny ip 192.168.10.0 0.0.0.31 10.0.0.0 0.255.255.255
 192 deny ip 192.168.10.0 0.0.0.31 172.16.0.0 0.15.255.255
 193 deny ip 192.168.10.0 0.0.0.31 192.168.0.0 0.0.255.255
 194 permit tcp 192.168.10.0 0.0.0.31 any eq 443
 200 deny ip any any
!
interface Vlan10
...
Note the bare `switchport port-security` on its own line — this is the master switch that actually enables the feature. The `maximum`, `mac-address sticky` and `violation` lines are only *parameters*; without the enabling command they are accepted into the configuration but do nothing, and `show port-security interface` reports `Port Security: Disabled` despite the parameters being visible in `show run`. This is the same two-level pattern as DHCP snooping (global switch plus per-VLAN scope), and an easy one to miss.
...
The **database agent** (`ip dhcp snooping database`) writes the binding table to flash so it survives a reload. This was added after testing revealed the problem directly: the binding table lives in RAM, so after a switch restart it is empty — and DAI, which depends on it, has nothing to validate against until every client renews its lease. With the agent configured, `show ip dhcp snooping database` confirms the table is read back at boot (`Successful Reads: 1`) and the bindings are restored without any client action.
...
spanning-tree vlan 10,20,101,123 root primary
```

`root primary` is a macro — it sets the bridge priority to 24576 for those VLANs. Priority moves in steps of 4096 because the Extended System ID reserves the lower 12 bits of the Bridge ID for the VLAN number, and 24576 is low enough to beat the 32768 default on the access switches. With the root at the distribution layer, STP blocks one leg of the SW1↔SW2 base to break the triangle loop. See [topology.md](topology.md) for the details.
